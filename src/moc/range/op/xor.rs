...
pub fn xor<T, Q, I1, I2>(left_it: I1, right_it: I2) -> XorRangeIter<T, Q, I1, I2>
where
  T: Idx,
  Q: MocQty<T>,
        ...
        (Some(ref mut l), Some(ref mut r)) => {
          if l.end == r.start {
            ...
            match l.start.cmp(&r.start) {
              ...
              Ordering::Less => {
                // L--R--L--R, L--LR--R return L increment L
                std::mem::swap(&mut l.end, &mut r.start);
                return std::mem::replace(&mut self.left, self.left_it.next());
              }
              Ordering::Greater => {
                // R--L--L--R, L--L R--R  return L increment L
                std::mem::swap(&mut l.end, &mut r.start);
            ...
            match l.start.cmp(&r.start) {
              ...
              Ordering::Less => {
                //  L--R--R--L, R--R L--L return R increment R
                std::mem::swap(&mut r.end, &mut l.start);
                std::mem::swap(&mut r.end, &mut r.start);
                return std::mem::replace(&mut self.right, self.right_it.next());
              }
              Ordering::Greater => {
                // R--L--R--L, R--RL--L  return R increment R
                std::mem::swap(&mut r.end, &mut l.start);
                return std::mem::replace(&mut self.right, self.right_it.next());
              }
            }
...
mod tests {
  ...
  use crate::moc::{
    range::op::xor::xor, CellMOCIntoIterator, CellMOCIterator, HasMaxDepth, RangeMOCIntoIterator,
    RangeMOCIterator,
  };
  use crate::qty::Hpx;
