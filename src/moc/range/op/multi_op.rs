use crate::idx::Idx;
use crate::moc::{range::MocRanges, RangeMOC, RangeMOCIterator};
use crate::qty::MocQty;

pub fn kway_or<'a, T, Q>(mut it: Box<dyn Iterator<Item = RangeMOC<T, Q>> + 'a>) -> RangeMOC<T, Q>
where
  T: Idx,
  Q: MocQty<T>,
{
...
pub fn kway_or_it<'a, T, Q, I>(mut it: Box<dyn Iterator<Item = I> + 'a>) -> RangeMOC<T, Q>
where
  T: Idx,
  Q: MocQty<T>,
...
pub fn kway_and<'a, T, Q>(mut it: Box<dyn Iterator<Item = RangeMOC<T, Q>> + 'a>) -> RangeMOC<T, Q>
where
  T: Idx,
  Q: MocQty<T>,
{
    ...
    Q1: MocQty<T1>,
  {
    ...
    fn next(&mut self) -> Option<Self::Item> {
      match (
        ...
        (Some(i1), Some(i2), Some(i3), None) => self.cur_moc.replace((i1.and(&i2)).and(&i3)),
        (Some(i1), Some(i2), None, _) => self.cur_moc.replace(i1.and(&i2)),
        (Some(i1), None, _, _) => self.cur_moc.replace(i1),
        (None, _, _, _) => self.cur_moc.take(),
      }
    }
  ...
  match (it.next(), it.next(), it.next(), it.next()) {
    ...
    (Some(i1), Some(i2), Some(i3), None) => (i1.and(&i2)).and(&i3),
    (Some(i1), Some(i2), None, _) => i1.and(&i2),
    (Some(i1), None, _, _) => i1,
    (None, _, _, _) => RangeMOC::new(0, MocRanges::default()),
  }
}

pub fn kway_and_it<'a, T, Q, I>(mut it: Box<dyn Iterator<Item = I> + 'a>) -> RangeMOC<T, Q>
where
  T: Idx,
  Q: MocQty<T>,
  I: RangeMOCIterator<T, Qty = Q>,
{
    ...
    I1: RangeMOCIterator<T1, Qty = Q1>,
  {
    ...
    fn next(&mut self) -> Option<Self::Item> {
      match (
        ...
        (Some(i1), Some(i2), Some(i3), Some(i4)) => self
          .cur_moc
          .replace((i1.and(i2)).and(i3.and(i4)).into_range_moc()),
        (Some(i1), Some(i2), Some(i3), None) => {
          self.cur_moc.replace((i1.and(i2)).and(i3).into_range_moc())
        }
        (Some(i1), Some(i2), None, _) => self.cur_moc.replace(i1.and(i2).into_range_moc()),
        (Some(i1), None, _, _) => self.cur_moc.replace(i1.into_range_moc()),
        (None, _, _, _) => self.cur_moc.take(),
      }
    }
  ...
  match (it.next(), it.next(), it.next(), it.next()) {
    ...
    (Some(i1), Some(i2), Some(i3), None) => (i1.and(i2)).and(i3).into_range_moc(),
    (Some(i1), Some(i2), None, _) => i1.and(i2).into_range_moc(),
    (Some(i1), None, _, _) => i1.into_range_moc(),
    (None, _, _, _) => RangeMOC::new(0, MocRanges::default()),
  }
}

pub fn kway_xor<'a, T, Q>(mut it: Box<dyn Iterator<Item = RangeMOC<T, Q>> + 'a>) -> RangeMOC<T, Q>
where
  T: Idx,
  Q: MocQty<T>,
{
    ...
    Q1: MocQty<T1>,
  {
    ...
    fn next(&mut self) -> Option<Self::Item> {
      match (
        ...
        (Some(i1), Some(i2), Some(i3), None) => self.cur_moc.replace((i1.xor(&i2)).xor(&i3)),
        (Some(i1), Some(i2), None, _) => self.cur_moc.replace(i1.xor(&i2)),
        (Some(i1), None, _, _) => self.cur_moc.replace(i1),
        (None, _, _, _) => self.cur_moc.take(),
      }
    }
  ...
  match (it.next(), it.next(), it.next(), it.next()) {
    ...
    (Some(i1), Some(i2), Some(i3), None) => (i1.xor(&i2)).xor(&i3),
    (Some(i1), Some(i2), None, _) => i1.xor(&i2),
    (Some(i1), None, _, _) => i1,
    (None, _, _, _) => RangeMOC::new(0, MocRanges::default()),
  }
}

pub fn kway_xor_it<'a, T, Q, I>(mut it: Box<dyn Iterator<Item = I> + 'a>) -> RangeMOC<T, Q>
where
  T: Idx,
  Q: MocQty<T>,
  I: RangeMOCIterator<T, Qty = Q>,
{
    ...
    I1: RangeMOCIterator<T1, Qty = Q1>,
  {
    ...
    fn next(&mut self) -> Option<Self::Item> {
      match (
        ...
        (Some(i1), Some(i2), Some(i3), Some(i4)) => self
          .cur_moc
          .replace((i1.xor(i2)).xor(i3.xor(i4)).into_range_moc()),
        (Some(i1), Some(i2), Some(i3), None) => {
          self.cur_moc.replace((i1.xor(i2)).xor(i3).into_range_moc())
        }
        (Some(i1), Some(i2), None, _) => self.cur_moc.replace(i1.xor(i2).into_range_moc()),
        (Some(i1), None, _, _) => self.cur_moc.replace(i1.into_range_moc()),
        (None, _, _, _) => self.cur_moc.take(),
      }
    }
  ...
  match (it.next(), it.next(), it.next(), it.next()) {
    ...
    (Some(i1), Some(i2), Some(i3), None) => (i1.xor(i2)).xor(i3).into_range_moc(),
    (Some(i1), Some(i2), None, _) => i1.xor(i2).into_range_moc(),
    (Some(i1), None, _, _) => i1.into_range_moc(),
    (None, _, _, _) => RangeMOC::new(0, MocRanges::default()),
  }
}
