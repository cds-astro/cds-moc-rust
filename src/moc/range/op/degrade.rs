...
#[inline(always)]
pub(crate) fn degrade_range<T: Idx>(
  range: &mut Range<T>,
  /*one_at_new_depth: T,*/ rm_bits_mask: T,
  bits_to_be_rm_mask: T,
) {
  ...
  range.end = (range.end + bits_to_be_rm_mask) & rm_bits_mask
}
  ...
  fn next(&mut self) -> Option<Self::Item> {
    if let Some(cr) = &mut self.curr {
      ...
      while let Some(nr) = &mut next {
        degrade_range(
          nr,
          /*self.one_at_new_depth,*/ self.rm_bits_mask,
          self.bits_to_be_rm_mask,
        );
        if nr.start > cr.end {
          break;
        } else {
