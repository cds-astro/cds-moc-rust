...
use super::{output::OutputFormat, InputTime};
...
pub enum From {
  ...
  Cone {
    ...
    r_deg: f64,
    #[structopt(subcommand)]
    out: OutputFormat, // add option: inside / overallaping / partially_in / centers_in
  },
    ...
    match self {
      ...
      From::MultiRegion {
        ...
        fn line2m(
          ...
          match geom.trim() {
            "cone" | "CONE" => {
              // center_lon_deg,center_lat_deg,radius_deg
              if params.len() != 3 {
                return Err(
                  format!(
                    "Wrong params. Expected: 3 (lon_deg,lat_deg,radius_deg); Actual: {}.",
                    params.len()
                  )
                  .into(),
                );
            ...
            "ellipse" | "ELLIPSE" => {
              ...
              if params.len() != 5 {
                return Err(
                  format!(
                    "Wrong params. Expected: 5 (lon_deg,lat_deg,a_deg,b_deg,pa_deg); Actual: {}.",
                    params.len()
                  )
                  .into(),
                );
            ...
            "polygon" | "POLYGON" => {
              ...
              if ((params.len() & 1) == 1) != complement {
                return Err(
                  format!(
                    "Wrong params. Expected: even value (two coo per vertex); Actual: {}.",
                    params.len()
                  )
                  .into(),
                );
              }
              let vertices_deg: Vec<f64> = params
                .iter()
                .map(|p| p.parse::<f64>())
            ...
            _ => Err(
              format!(
                "Unrecognized geometry {}.Expected: cone, ellipse, rng, box, zone  oe polygon",
                geom
              )
              .into(),
            ),
      ...
      From::ValuedCells {
        ...
        fn line2uvd_from_val(
          separator: &str,
          depth: u8,
          line: std::io::Result<String>,
        ) -> Result<(u64, f64, f64), Box<dyn Error>> {
          ...
          if cdepth > depth {
            return Err(
              format!(
                "Cell depth {} larger than MOC depth {} not supported.",
                cdepth, depth
              )
              .into(),
            );
        ...
        let ranges: HpxRanges<u64> = if density {
          let line2uniq_val_dens = move |line: std::io::Result<String>| match line2uvd_from_dens(
            &separator, depth, line,
          ) {
      ...
      From::ValuedCells {
        ...
        revese_recursive_descent,
        input_type:
          MultiOrderInputFormat::FitsSkymap {
            skip_vals_le_than,
            input,
            out,
          },
      ...
      From::Timestamp {
        ...
        let line2ts = move |line: std::io::Result<String>| match line
          .map_err(|e| e.into())
          .and_then(|s| time.parse(&s))
        {
          Ok(t) => Some(t),
          Err(e) => {
            error!("Error reading or parsing line: {:?}", e);
        ...
        if input == PathBuf::from(r"-") {
          ...
          out.write_tmoc_possibly_auto_converting_from_u64(
            RangeMOC::<u64, Time<u64>>::from_microsec_since_jd0(
              depth,
              stdin.lock().lines().filter_map(line2ts),
              None,
            )
            .into_range_moc_iter(),
          )
          ...
          out.write_tmoc_possibly_auto_converting_from_u64(
            RangeMOC::<u64, Time<u64>>::from_microsec_since_jd0(
              depth,
              reader.lines().filter_map(line2ts),
              None,
            )
            .into_range_moc_iter(),
          )
        ...
        if input == PathBuf::from(r"-") {
          ...
          out.write_tmoc_possibly_auto_converting_from_u64(
            RangeMOC::<u64, Time<u64>>::from_microsec_ranges_since_jd0(
              depth,
              stdin.lock().lines().filter_map(line2trange),
              None,
            )
            .into_range_moc_iter(),
          )
          ...
          out.write_tmoc_possibly_auto_converting_from_u64(
            RangeMOC::<u64, Time<u64>>::from_microsec_ranges_since_jd0(
              depth,
              reader.lines().filter_map(line2trange),
              None,
            )
            .into_range_moc_iter(),
          )
      ...
      From::TimestampPos {
        ...
        let line2tpos = move |line: std::io::Result<String>| match line2tscoos(
          &separator, &time, layer, time_shift, line,
        ) {
          Ok(lonlat) => Some(lonlat),
          Err(e) => {
            error!("Error reading or parsing line: {:?}", e);
        ...
        let moc2: RangeMOC2<u64, Time<u64>, u64, Hpx<u64>> = if input == PathBuf::from(r"-") {
          let stdin = std::io::stdin();
          RangeMOC2::from_fixed_depth_cells(
            tdepth,
            sdepth,
            stdin.lock().lines().filter_map(line2tpos),
            None,
          )
        } else {
          let f = File::open(input)?;
          ...
          RangeMOC2::from_fixed_depth_cells(
            tdepth,
            sdepth,
            reader.lines().filter_map(line2tpos),
            None,
          )
        };
      ...
      From::TimerangePos {
        ...
        let moc2: RangeMOC2<u64, Time<u64>, u64, Hpx<u64>> = if input == PathBuf::from(r"-") {
          let stdin = std::io::stdin();
          RangeMOC2::from_ranges_and_fixed_depth_cells(
            tdepth,
            sdepth,
            stdin.lock().lines().filter_map(line2trpos),
            None,
          )
        } else {
          let f = File::open(input)?;
          ...
          RangeMOC2::from_ranges_and_fixed_depth_cells(
            tdepth,
            sdepth,
            reader.lines().filter_map(line2trpos),
            None,
          )
        };
        out.write_stmoc(moc2.into_range_moc2_iter())
      }
      From::FreqValue { depth, input, out } => {
        ...
        if input == PathBuf::from(r"-") {
          ...
          out.write_fmoc_possibly_auto_converting_from_u64(
            RangeMOC::<u64, Frequency<u64>>::from_freq_in_hz(
              depth,
              stdin.lock().lines().filter_map(line2freq),
              None,
            )
            .into_range_moc_iter(),
          )
          ...
          out.write_fmoc_possibly_auto_converting_from_u64(
            RangeMOC::<u64, Frequency<u64>>::from_freq_in_hz(
              depth,
              reader.lines().filter_map(line2freq),
              None,
            )
            .into_range_moc_iter(),
          )
      ...
      From::FreqRange {
        ...
        fn line2tr(
          separator: &str,
          line: std::io::Result<String>,
        ) -> Result<Range<f64>, Box<dyn Error>> {
          let line = line?;
          let (fmin, fmax) = line
            .trim()
            .split_once(separator)
