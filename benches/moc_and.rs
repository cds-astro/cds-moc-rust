...
fn bench_and(c: &mut Criterion) {
  ...
  group.bench_function("Ranges INTERSECTION", |b| {
    b.iter(|| test_and_ranges(sdss.clone(), other.clone()))
  });
  group.bench_function("Ranges Iter AND", |b| {
    b.iter(|| test_and_ranges_it(sdss.clone(), other.clone()))
  });
  group.bench_function("Ranges Ref Iter AND", |b| {
    b.iter(|| test_and_ranges_it_ref(sdss.clone(), other.clone()))
  });
  group.bench_function("BMOC AND", |b| {
    b.iter(|| {
      test_and_bmoc(
