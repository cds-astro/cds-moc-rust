...
fn bench_or(c: &mut Criterion) {
  ...
  group.bench_function("Ranges UNION", |b| {
    b.iter(|| test_or_ranges(sdss.clone(), other.clone()))
  });
  group.bench_function("Ranges Iter OR", |b| {
    b.iter(|| test_or_ranges_it(sdss.clone(), other.clone()))
  });
  group.bench_function("Ranges Ref Iter OR", |b| {
    b.iter(|| test_or_ranges_it_ref(sdss.clone(), other.clone()))
  });
